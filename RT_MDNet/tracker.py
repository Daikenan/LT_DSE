...
def set_optimizer(model, lr_base, lr_mult=rt_opts['lr_mult'], momentum=rt_opts['momentum'], w_decay=rt_opts['w_decay']):
...
def rt_train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    model.train()

    batch_pos = rt_opts['batch_pos']
    batch_neg = rt_opts['batch_neg']
    batch_test = rt_opts['batch_test']
    batch_neg_cand = max(rt_opts['batch_neg_cand'], batch_neg)
    ...
    for iter in range(maxiter):
        ...
        model.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm(model.parameters(), rt_opts['grad_clip'])
        optimizer.step()

        if rt_opts['visual_log']:
...
def run_mdnet(img_list, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='', display=False):
    ...
    # Init model
    model = MDNet(rt_opts['model_path'])
    ...
    model.set_learnable_params(rt_opts['ft_layers'])
    ...
    init_optimizer = set_optimizer(model, rt_opts['lr_init'])
    update_optimizer = set_optimizer(model, rt_opts['lr_update'])
    ...
    for bidx in range(0,scene_boxes.shape[0]):
        crop_img_size = (scene_boxes[bidx,2:4] * ((rt_opts['img_size'],rt_opts['img_size'])/target_bbox[2:4])).astype('int64')*jitter_scale[bidx]
        ...
        cur_pos_rois = samples2maskroi(cur_pos_rois, model.receptive_field,(scaled_obj_size,scaled_obj_size), target_bbox[2:4], rt_opts['padding'])
        ...
        cur_neg_rois = samples2maskroi(cur_neg_rois, model.receptive_field, (scaled_obj_size,scaled_obj_size), target_bbox[2:4], rt_opts['padding'])
        ...
        scaled_obj_size = float(rt_opts['img_size'])*jitter_scale[bidx]
        cur_bbreg_rois = samples2maskroi(cur_bbreg_rois, model.receptive_field,(scaled_obj_size,scaled_obj_size), target_bbox[2:4], rt_opts['padding'])
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_pos_examples = gen_samples(SampleGenerator('gaussian', (ishape[1], ishape[0]), 0.1, 1.2),extra_target_bbox, rt_opts['n_pos_init']/replicateNum, rt_opts['overlap_pos_init'])
        cur_extra_neg_examples = gen_samples(SampleGenerator('uniform', (ishape[1], ishape[0]), 0.3, 2, 1.1),extra_target_bbox, rt_opts['n_neg_init']/replicateNum/4, rt_opts['overlap_neg_init'])

        ##bbreg sample
        cur_extra_bbreg_examples = gen_samples(SampleGenerator('uniform', (ishape[1], ishape[0]), 0.3, 1.5, 1.1),extra_target_bbox, rt_opts['n_bbreg']/replicateNum/4, rt_opts['overlap_bbreg'], rt_opts['scale_bbreg'])
        ...
        cur_extra_pos_rois = samples2maskroi(cur_extra_pos_rois, model.receptive_field,(scaled_obj_size, scaled_obj_size), extra_target_bbox[2:4], rt_opts['padding'])
        ...
        cur_extra_neg_rois = samples2maskroi(cur_extra_neg_rois, model.receptive_field,(scaled_obj_size, scaled_obj_size), extra_target_bbox[2:4], rt_opts['padding'])
        ...
        cur_extra_bbreg_rois = samples2maskroi(cur_extra_bbreg_rois, model.receptive_field,(scaled_obj_size,scaled_obj_size), extra_target_bbox[2:4], rt_opts['padding'])
    ...
    # Initial training
    train(model, criterion, init_optimizer, pos_feats, neg_feats, rt_opts['maxiter_init'])
    ...
    if pos_feats.size(0) > rt_opts['n_pos_update']:
        pos_idx = np.asarray(range(pos_feats.size(0)))
        np.random.shuffle(pos_idx)
        pos_feats_all = [pos_feats.index_select(0, torch.from_numpy(pos_idx[0:rt_opts['n_pos_update']]).cuda())]
    if neg_feats.size(0) > rt_opts['n_neg_update']:
        neg_idx = np.asarray(range(neg_feats.size(0)))
        np.random.shuffle(neg_idx)
        neg_feats_all = [neg_feats.index_select(0, torch.from_numpy(neg_idx[0:rt_opts['n_neg_update']]).cuda())]
    ...
    for i in range(1,len(img_list)):
        ...
        ishape = cur_image.shape
        samples = gen_samples(SampleGenerator('gaussian', (ishape[1], ishape[0]), trans_f, rt_opts['scale_f'],valid=True), target_bbox, rt_opts['n_samples'])
        ...
        crop_img_size = (padded_scene_box[2:4] * ((rt_opts['img_size'], rt_opts['img_size']) / target_bbox[2:4])).astype(
            'int64')
        ...
        sample_rois = samples2maskroi(sample_rois,model.receptive_field, (rt_opts['img_size'],rt_opts['img_size']), target_bbox[2:4],rt_opts['padding'])
        ...
        if success:
            ...
            trans_f = rt_opts['trans_f_expand']
        ...
        if success:
            ...
            if len(pos_feats_all) > rt_opts['n_frames_long']:
                del pos_feats_all[0]
            if len(neg_feats_all) > rt_opts['n_frames_short']:
                del neg_feats_all[0]

        # Short term update
        if not success:
            nframes = min(rt_opts['n_frames_short'],len(pos_feats_all))
            pos_data = torch.stack(pos_feats_all[-nframes:],0).view(-1,feat_dim)
            neg_data = torch.stack(neg_feats_all,0).view(-1,feat_dim)
            train(model, criterion, update_optimizer, pos_data, neg_data, rt_opts['maxiter_update'])

        # Long term update
        elif i % rt_opts['long_interval'] == 0:
