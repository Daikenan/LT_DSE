...
class SSDMetaArch(model.DetectionModel):
  ...
  def postprocess(self, prediction_dict):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      detection_boxes_d = tf.expand_dims(detection_boxes, axis=2)
      ...
      (nmsed_boxes, nmsed_scores, nmsed_classes, _,
       num_detections) = self._non_max_suppression_fn(detection_boxes_d,
                                                      detection_scores,
                                                      clip_window=clip_window)
      return {'detection_boxes': nmsed_boxes,
              'detection_scores': nmsed_scores,
              'detection_classes': nmsed_classes,
              'num_detections': tf.to_float(num_detections)}, detection_boxes, detection_scores
