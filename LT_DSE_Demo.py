...
class MobileTracker(object):
    ...
    def tracking(self, image):
        ...
        if self.p.visualization:
            show_res(cv.cvtColor(image, cv.COLOR_RGB2BGR), np.array(outputs, dtype=np.int32), '2',
        ...
        return [float(outputs[1]), float(outputs[0]), float(outputs[3]-outputs[1]), float(outputs[2]-outputs[0])], confidence_score
...
def eval_tracking(Dataset, video_spe=None, save=False, p=None):
    if Dataset == "votlt":
        ...
        data_dir = '/home/daikenan/dataset/LASOT/LaSOTBenchmark/volleyball'
    elif Dataset == 'demo':
        data_dir = './seq'
    ...
    for seq_id, video in enumerate(sequence_list):
        if Dataset == "votlt" or Dataset == "votlt19" or Dataset == 'demo':
            ...
            sequence_dir = data_dir + '/' + video + '/img/'
            gt_dir = data_dir + '/' + video + '/groundtruth.txt'
        result_save_path = os.path.join(p.save_path, p.name, video+'.txt')
        if save and not os.path.exists(os.path.join(p.save_path, p.name)):
            os.makedirs(os.path.join(p.save_path, p.name))
...
def main(_):
    p = p_config()
    p.name = 'LT_DSE'
    eval_tracking('demo', p=p, save=True)
